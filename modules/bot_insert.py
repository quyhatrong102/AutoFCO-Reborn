...
class InsertMixin:
    ...
    def _ocr_insert_from_img(self, img, debug_label=""):
        try:
            iw, ih = img.size
    ...
    def _wait_popup_close(self, timeout=3.0):
    ...
    def _price_click_coords(self, is_max):
        return (973, 321) if is_max else (975, 329)
    ...
    def run_insert(self, slot_configs, do_notify=False):
        self.running = True
        ...
        try:
            ...
            # Click vao tab Danh sach yeu thich
            ...
            while self.running:
                if not self._in_active_window():
                    s_now = self._get_clock_now().second
                    self.log(f"⏸ Qua :25 — chờ đến :50... (hiện tại :{s_now:02d})", "orange")
                    while self.running:
                        if self._get_clock_now().second >= 50:
                            break
                        time.sleep(0.05)
                ...
                for cfg in active_slots:
                    ...
                    if is_max:
                        pyautogui.click(x1 + action_x, y1 + action_y)
                        time.sleep(0.1)
                    else:
                        btn_ready = self.hover_and_wait_color(
                            action_x, action_y, "2554EA", timeout=30.0, click_if_match=True
                        )
                        if not btn_ready:
                            continue
    
                    self._wait_popup_close()
                    ...
                    if is_max:
                        price_img = ImageGrab.grab(bbox=(x1+908, y1+319, x1+1063, y1+340))
                    else:
                        price_img = ImageGrab.grab(bbox=(x1+908, y1+319, x1+1063, y1+340))
                    label = f"slot{slot_num}_{'max' if is_max else 'min'}"
                    price_now = self._ocr_insert_from_img(price_img, debug_label=label)
                    ...
                    if price_now and price_now != st["init_price"]:
                        ...
                        px, py = self._price_click_coords(is_max)
                        pyautogui.click(x1 + px, y1 + py)
                        time.sleep(0.1)
            ...
            self.log(f"   ❌ [LỖI NGHIÊM TRỌNG TRONG AUTO CHÈN] {e}", "fail")
            self.log(f"Chi tiết lỗi: {err_msg}", "fail")
            safe_finish(False)
